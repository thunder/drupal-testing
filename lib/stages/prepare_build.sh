...
_stage_prepare_build() {
    ...
    echo "$( jq '.repositories[0].options = {}' ${DRUPAL_TESTING_DRUPAL_INSTALLATION_DIRECTORY}/composer.json )" > "${DRUPAL_TESTING_DRUPAL_INSTALLATION_DIRECTORY}""/composer.json"
    echo "$( jq '.repositories[0].options.symlink = false' ${DRUPAL_TESTING_DRUPAL_INSTALLATION_DIRECTORY}/composer.json )" > "${DRUPAL_TESTING_DRUPAL_INSTALLATION_DIRECTORY}""/composer.json"
