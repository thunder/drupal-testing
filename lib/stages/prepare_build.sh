...
_stage_prepare_build() {
    ...
    if ${DRUPAL_TESTING_TEST_DEPRECATION}; then
        composer require phpstan/phpstan:0.12.42 --no-update --working-dir="${DRUPAL_TESTING_DRUPAL_INSTALLATION_DIRECTORY}"
    ...
    # Enable patching
    composer require cweagans/composer-patches:1.x-dev --no-update --working-dir="${DRUPAL_TESTING_DRUPAL_INSTALLATION_DIRECTORY}"
