...
_stage_setup() {
    ...
    if  ! port_is_open ${DRUPAL_TESTING_SELENIUM_HOST} ${DRUPAL_TESTING_SELENIUM_PORT}; then
        ...
        if ${DRUPAL_TESTING_USE_SELENIUM} = true; then
            docker run --detach --net host --name ${DRUPAL_TESTING_SELENIUM_DOCKER_NAME} --volume /dev/shm:/dev/shm selenium/standalone-chrome:${DRUPAL_TESTING_SELENIUM_CHROME_VERSION}
        else
            download_chromedriver
